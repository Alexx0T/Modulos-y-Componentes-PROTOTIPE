# Interfaz de Chat de Agente (AgentChatInterface)

Componente premium de chat conversacional con visualizadores interactivos para llamadas a herramientas (`Tool Executions`) y optimización de repaints por hardware.

---

## 1. Propósito y Casos de Uso
* **Flujos de Razonamiento:** Facilita la comprensión del usuario al exponer las llamadas a herramientas y scripts ejecutados internamente por los agentes conversacionales de IA.
* **Aceleración por GPU:** Implementa `will-change: transform` y `backface-visibility` para mitigar redibujados del DOM durante flujos continuos de streaming de texto.
* **Casos de Uso:**
  * Terminal conversacional de agentes en interfaces SaaS.
  * Consola de depuración de flujos de IA en tiempo real.

---

## 2. Especificación Visual (Tailwind CSS HSL)
* **Diseño Glass-Premium:** Combinación de bordes semi-transparentes (`border-white/5`), fondos suavizados por blur y botones con efectos de hover e iluminación HSL.
* **Iconografía SVG Nativa:** Todos los iconos de envío y reciclado son inyectados como elementos `<svg>` inline con el atributo `currentColor`.
...
|------|------|---------|-------------|
| `messages` | `array` | - | Colección estructurada de mensajes con estado y marca de tiempo. |
| `onSendMessage` | `function` | - | Callback ejecutado cuando el usuario envía un prompt. |
| `onClearChat` | `function` | - | Callback de limpieza del historial. |
| `isTyping` | `boolean` | `false` | Activa la animación con efecto bounce del agente. |

---

## 4. Código React Fuente Completo (`AgentChatInterface.jsx`)
```jsx
...
## 5. Origen
* **Extraído de:** `src/components/AgentChatInterface.jsx`
