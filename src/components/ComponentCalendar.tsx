...
import type { RegisteredComponent } from '../store/useCatalogStore'

interface ComponentCalendarProps {
  components: RegisteredComponent[]
}

export const ComponentCalendar: React.FC<ComponentCalendarProps> = ({ components }) => {
  // Use current date as calendar anchor (June 2026 for the current system time)
  const [currentYear] = useState(2026)
  const [currentMonth] = useState(5) // June (0-indexed in JS Dates)
  const [selectedDayComponents, setSelectedDayComponents] = useState<RegisteredComponent[]>([])
  ...
  // June 1st, 2026 was a Monday (index 1)
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 p-6 rounded-2xl border border-zinc-900 bg-zinc-900/10 min-h-[420px] gpu-accelerated">
      ...
      <div className="md:col-span-7 space-y-4">
        ...
        {/* Days Grid - CLS-safe fixed dimensions */}
